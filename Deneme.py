...
class WellcomeRPAApp(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self._build_ui()  
        self._update_ui_texts()
        logger.info("Application initialized with explicit unhandled prompt handler.")
    ...
    def _create_driver(self) -> Optional[uc.Chrome]:
        ...
        try:
            ...
            backup_options.page_load_strategy = 'eager'
    ...
    def _execute_document_upload_pipeline(self, driver: uc.Chrome, bekleme: WebDriverWait, row: pd.Series, docs_folder: str) -> bool:
        ...
        try:
            self._log("🚀 Tokenli evrak yükleme paneline yönlendiriliyor...", "system")
            target_upload_url = "https://wellcome.azurewebsites.net/pnlwell/files/documents/?v=CyGOrRq/t88nmeL78K/hmA==&dcode=ce"
            
            driver.execute_script(f"window.location.href = '{target_upload_url}';")
            time.sleep(3.5 + user_delay)
            
            bekleme = WebDriverWait(driver, 25)
            bekleme.until(EC.presence_of_element_located((By.ID, "drpWorker")))

            self._log("🧬 Personel listesi taranıyor ve kilitleniyor...", "system")
            js_worker_injector = """
            ...
            if not success_match:
                self._log(f"❌ Dropdown listesinde {ad_soyadi} (TC: {tc_no}) personeli bulunamadı!", "error")
                return False

            self._log(f"🎯 Otomasyon hedef personeli kilitledi: {ad_soyadi}", "success")
            ...
            for opt in select_obj.options:
                ...
                if val and val != "0|N" and opt.text.strip():
                    evrak_tipleri.append((opt.text.strip(), val))

            evrak_haritasi = {
                "Adli Sicil Belgesi": ["ADLI", "SICIL", "SABIKA"],
                "Nüfus Cüzdanı Sadece Ön Yüz": ["KIMLIK", "NUFUS", "CILD", "EHLIYET", "ON YUZ", "KIMLIK_ON"],
            ...
            for t_text, t_val in evrak_tipleri:
                uygun_dosya = None
                
                for anahtar_tanim, kelimeler in evrak_haritasi.items():
                    if anahtar_tanim.upper() in t_text.upper() or t_text.upper() in anahtar_tanim.upper():
                        ...
                        if dosya.upper().endswith('.PDF'):
                            try:
                                ...
                                with pdfplumber.open(full_p) as pdf:
                                    ...
                                    if "ADLİ SİCİL" in ilk_sayfa_metni and "ADLI" in t_text.upper():
                                        uygun_dosya = dosya; break
                                    elif "SGK" in ilk_sayfa_metni and "SGK" in t_text.upper():
                                        uygun_dosya = dosya; break
                                    elif "SAĞLIK RAPORU" in ilk_sayfa_metni and "HEKIM" in t_text.upper():
                                        uygun_dosya = dosya; break
                            except: pass
                ...
                if uygun_dosya:
                    ...
                    self._log(f"📁 [YÜKLENDİ] -> Tip: '{t_text[:20]}...' -> Dosya: {uygun_dosya}", "success")
                    time.sleep(user_delay + 3.0)
            ...
            self._log(f"⚠️ Evrak yükleme boru hattında arıza: {str(e)[:50]}", "warning")
            return False
    ...
    def _process_single_row(self, driver: Optional[uc.Chrome], row: pd.Series, docs_folder: str) -> Tuple[str, Dict[str, str]]:
        ...
        if self.only_docs_mode_var.get():
            self._log(f"🚀 'Sadece Evrak Modu' Aktif -> {personel_ismi} için kayıt adımı atlanıyor...", "system")
            bekleme = WebDriverWait(driver, 20)
    ...
    def _run_automation_loop(self, df_data: pd.DataFrame, file_path: str, docs_folder: str) -> None:
        ...
        try:
            ...
            for index, (_, row) in enumerate(df_data.iterrows()):
                ...
                # --- SABİTLENEN TEMİZ TEKİL RAPORLAMA BLOGU ---
                results_report.append({
                    "Personel Ad Soyad": personel_data["isim_soyisim"],
                    "TC Kimlik No": self._mask_sensitive_data(personel_data["tc"], is_tc=True),
                    "Telefon No": self._mask_sensitive_data(personel_data["telefon"], is_tc=False),
