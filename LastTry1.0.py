...
class WellcomeRPAApp(ctk.CTk):
    ...
    # =====================================================================
    # --- KOORDİNAT OOFFSET DESTEKLİ VE INSANSI KLAVYE SÜRÜMÜ ---
    # =====================================================================
        ...
        try:
            ...
            for tip in evrak_tipleri:
                ...
                if uygun_dosya:
                    ...
                    # --- [KÖKTEN DÜZELTME]: X (CLEAR) BUTONUNDAN KAÇAN ÇEPER OFFSET TIKLAMASI ---
                    ...
                    # Aksiyon zincirini kuruyoruz: Elementin tam ortasından sola doğru 50 piksel kayarak (X butonundan kurtulup) çepere tıklatıyoruz
                    actions = ActionChains(driver)
                    actions.move_to_element_with_offset(select2_container, -50, 0).click().perform()
                    time.sleep(0.5)

                    # Garanti zırhı: Dropdown hala uyanmadıysa elemente odaklanıp klavyeden SPACE (Boşluk) veya DOWN göndererek listeyi patlatıyoruz
                    try:
                        select2_container.send_keys(Keys.SPACE)
                    except: pass
                    time.sleep(1.0)

                    # 2. Açılan arama kutusuna odaklan (Artık kutu %100 açık)
                    ...
                    # 3. İsmi karakter karakter (İnsansı hız simülasyonu) yazdır
                    ...
                    # Arka plandaki ham select elementini JS enjeksiyonu ile senkronize et
                    js_worker_injector = """
                    ...
                    self._log(f"🎯 Personel otomatik olarak kilitlendi: {ad_soyadi}", "success")

                    # Evrak tipini seç ve yüklemeyi tamamla
                    evrak_select_el = bekleme.until(EC.presence_of_element_located((By.ID, "drpEvrak")))
                    select_obj = Select(evrak_select_el)
