...
class WellcomeRPAApp(ctk.CTk):
    ...
    # =====================================================================
    # --- ÇALIŞAN ESKİ DOĞRUDAN SÜRÜM + SEÇİM ALANI SIMÜLASYONU ---
    # =====================================================================
    def _execute_document_upload_pipeline(self, driver: uc.Chrome, bekleme: WebDriverWait, row: pd.Series, docs_folder: str) -> bool:
        ...
        try:
            # --- ESKİ ÇALIŞAN DOĞRUDAN GET PATİKASI ---
            ...
            # --- ARANAN SELECT2 SEÇİMİNİ DESTEKLEYEN YENİ HİBRİT YAPILANDIRMA ---
            self._log("🧬 Personel listesi kilitleniyor...", "system")
            
            # 1. Arayüzdeki Select2 container'ını bul ve fiziki olarak tıkla (Kutuyu açmak için)
            select2_container = bekleme.until(EC.element_to_be_clickable((By.XPATH, "//*[contains(@class, 'select2-selection') and @aria-labelledby='select2-drpWorker-container']")))
            ...
            # 2. Arka plandaki ham select nesnesini bulup JS ile veriyi basıyoruz ve change event'ini fırlatıyoruz
            js_worker_injector = """
                ...
                for (var i = 0; i < select_el.options.length; i++) {
                    ...
                    if (opt.text.indexOf(target_text_1) !== -1 || opt.text.indexOf(target_text_2) !== -1) {
                        ...
                        // Select2 kütüphanesine ham elementin değiştiğini haber ver
            ...
            if not success_match:
                self._log(f"❌ Personel arama listesinde bulunamadı: {ad_soyadi}", "error")
                return False

            self._log(f"🎯 Personel başarıyla kilitlendi: {ad_soyadi}", "success")

            # --- EVRAK TİPLERİ OKUMA VE DOSYA ENJEKSİYONU ---
            ...
            for tip in evrak_tipleri:
                        ...
                        if dosya.upper().endswith('.PDF'):
                            try:
                                ...
                                with pdfplumber.open(full_p) as pdf:
                                    ...
                                    if "ADLİ SİCİL" in ilk_sayfa_metni and "ADLI" in tip.upper():
                ...
                if uygun_dosya:
                    ...
                    self._log(f"📁 [YÜKLENDİ] -> Tip: '{tip[:20]}...' -> Dosya: {uygun_dosya}", "success")
    ...
    def _process_single_row(self, driver: Optional[uc.Chrome], row: pd.Series, docs_folder: str) -> Tuple[str, Dict[str, str]]:
        ...
        for deneme in range(1, 4):
            try:
                ...
                # --- ARIZALI BEKLEME KALDIRILDI - DOĞRUDAN ESKİ STABİL PATİKA ---
                driver.execute_script("arguments[0].click();", kaydet_btn)
                time.sleep(5.0 + user_delay) # Post işleminin tamamlanması için güvenli duraklama süresi
