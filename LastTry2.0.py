...
LANG_PACK = {
    "TR": {
        ...
        "subtitle": "Enterprise RPA Control Panel v3.0 AI",
...
class WellcomeRPAApp(ctk.CTk):
    ...
    def _analyze_document_content_ai(self, file_path: str) -> Optional[str]:
        ...
        if not text_content.strip():
            try:
                ...
                self._log(f"⚠️ OCR Motorunda anlık sapma (Sistem koruması devrede)", "warning")

        text_content = text_content.upper().replace('İ', 'I')

        # Genişletilmiş ve Türkçe Karakter Güvenliği Alınmış Matris
        evrak_ai_weights = {
            "Adli Sicil Belgesi": ["ADLI SICIL", "SABIKA KAYDI", "ARSIV KAYDI", "ADALET BAKANLIGI", "CEZA DAIRESI", "ADLI", "SICIL", "SABIKA"],
            "Nüfus Cüzdanı Sadece Ön Yüz": ["T.C. KIMLIK KARTI", "NUFUS CUZDANI", "DOGUM TARIHI", "SERI NO", "ANA ADI", "BABA ADI", "KIMLIK NUMARASI", "KIMLIK", "UYRUGU"],
            ...
            "İş Sağlığı ve Güvenliği Talimat ve Taahhütnamesi": ["TALIMAT VE TAAHHUTNAMESI", "TAAHHUTNAME", "MADDELERINI", "TEBELLUG", "TALIMATNAME", "OZGU TALIMATNAME", "TAAHHUT", "TALIMAT"]
        }
    ...
    # =====================================================================
    # --- JS ENJEKSİYON DESTEKLİ VE KAYMAYI ÖNLEYEN EVRAK MOTORU ---
    # =====================================================================
    def _execute_document_upload_pipeline(self, driver: uc.Chrome, bekleme: WebDriverWait, row: pd.Series, docs_folder: str) -> bool:
        ...
        try:
            ...
            # Sitedeki ham option elementlerinin value ve text değerlerini çekiyoruz
            ...
            for dosya in mevcut_dosyalar:
                ...
                self._log(f"🧠 AI Analiz Ediyor -> {dosya}...", "system")
                ...
                # SELECT2 AD SOYAD KİLİTLEME PROSESİ (OFFSET TIKLAMA)
                bekleme = WebDriverWait(driver, 25)
                ...
                try: select2_container.send_keys(Keys.SPACE)
                except: pass
                time.sleep(1.0)
                ...
                # --- [KİLİT REVIZYONU]: KAYMAYI ÖNLEN KISMİ KELİME KÖKÜNDEN VALUE BULUCU ---
                hedef_value = None
                hedef_görünür_metin = ""
                ...
                if not hedef_value:
                    kisa_kok = tahmin_edilen_tip.split()[0] # "SGK" vb.
                    for val, text in sitedeki_opsiyonlar:
                ...
                # --- [ZIRH_3]: SEÇİMİ DIŞARIDAN JS ENJEKSİYONU İLE YAPARAK KAYMAYI SIFIRLAMA ---
                try:
                    js_dropdown_force = """
                    ...
                    self._log(f"❌ Dropdown kilit hatası: {hedef_görünür_metin}", "error")
                    continue
